...
function FindFriendItem(props: FriendItemType) {
    ...
    const onFollow = () => {
        axios.post(`https://social-network.samuraijs.com/api/1.0/follow/${props.users.id}`, {}, {withCredentials: true, headers:{"API-KEY":'3de0712e-e19a-4637-bf5e-355c7a8ad46c'}})
            .then(response => {
    ...
    const onUnfollow = () => {
        axios.delete(`https://social-network.samuraijs.com/api/1.0/follow/${props.users.id}`, {withCredentials: true,headers:{"API-KEY":'3de0712e-e19a-4637-bf5e-355c7a8ad46c'}})
            .then(response => {
                if (response.data.resultCode == 0) {
            ...
            <div className={obj.info_grid}>
                ...
                <div className={obj.follow_wrapper}>
                    ...
                    {props.users.followed ?
                        <button onClick={onUnfollow} className={obj.unfollow_btn}>Unfollow</button>
                        :
                        <button onClick={onFollow} className={obj.follow_btn}>Follow</button>
                    }
                </div>
            </div>
        </div>
